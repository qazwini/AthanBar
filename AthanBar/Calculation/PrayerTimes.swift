...
class PrayerTimes {
    func getPrayerEntries(forDate date: Date = Date()) -> [PrayerEntry] {
        calcDate = date
        let times = getPrayerTimes()!
        return Prayer.allDefaultCases.compactMap {
            if let prayerDate = times[TimeNames(rawValue: $0.rawValue)!] as? Date {
                return PrayerEntry(prayer: $0, date: prayerDate)
            }
            return nil
        }
    }
    
    func getNextPrayerEntry(forDate date: Date = Date()) -> PrayerEntry {
        let currDayEntries = getPrayerEntries(forDate: date)
        if let nextPrayer = currDayEntries.first(where: { $0.date > date }) {
            return nextPrayer
        } else {
            let nextDayEntries = getPrayerEntries(forDate: date.dayAfter)
            return nextDayEntries.first(where: { $0.date > date })!
        }
    }
    ...
    enum CalculationMethod: String {
        ...
        static let defaultCases: [CalculationMethod] = [.Jafari, .Tehran]
    }
    ...
    */
    ...
    var calculationMethod      = CalculationMethod.Jafari
    ...
    var outputFormat           = OutputTimeFormat.Date
    ...
    var timeZone: Float = PrayerTimes.systemTimeZone()
    ...
    private lazy var jDate:Double = PrayerTimes.julianDate(from: Date())
    ...
    class func dayLightSavingOffset(forDate date: Date = Date()) -> Double {
        let timeZone = TimeZone.current
    ...
    func getDatePrayerTimes(year:Int, month:Int, day:Int, latitude:Double, longitude:Double, tZone:Float)->[TimeNames: Any] {
        ...
        comp.month = month
        comp.day = day
        calcDate = PrayerTimes.gregorianCalendar.date(from: comp)
        
        timeZone = tZone
        
        jDate = PrayerTimes.julianDate(year: year, month: month, day: day)
    ...
    func floatToHourMinute(_ time:Double)->(hours:Int, minutes:Int)? {
        if time.isNaN { return nil }
        
        let ttime = time + 0.5 / 60.0//fixHour(time + 0.5 / 60.0)  // add 0.5 minutes to round
    ...
    func floatToTime24(_ time: Double) -> String {
        guard let (hours, minutes) = floatToHourMinute(Double(time)) else { return "---" }
    ...
    func floatToTime12(_ time:Double, noSuffix:Bool)->String {
        ...
        var updatedHours = (hours == 0) ? 12 : hours
        if minutes == 60 { updatedHours += 1 }
        if updatedHours > 12 { updatedHours -= 12 }
        let updatedMinutes = (minutes == 60) ? 0 : minutes
        
        return NSString(format: "%01d:%02d%@", updatedHours, updatedMinutes, (noSuffix ? "" : ((hours > 12) ? " PM" : " AM")) ) as String
    }
    
    /// Convert float hours to NSDate
    func floatToNSDate(_ time:Double)->Date? {
        if let (hours, minutes) = floatToHourMinute(time) {
            var components = PrayerTimes.gregorianCalendar.dateComponents(PrayerTimes.calendarComponentsDMY, from: calcDate)
            components.hour = hours
            components.minute = minutes
            return PrayerTimes.gregorianCalendar.date(from: components)
        } else {
            return nil
        }
    }
    ...
    private func calculateJulianDate() {
        guard let date = calcDate, let latlng = coordinate else { return }
        jDate = PrayerTimes.julianDate(from: date)
        jDate = jDate - (latlng.longitude / (15.0 * 24.0))
    }
    
    private class func julianDate(from date:Date)->Double {
        let components = PrayerTimes.gregorianCalendar.dateComponents(PrayerTimes.calendarComponentsDMY, from: date)
    ...
    // compute mid-night (Midnight) time
    private func computeMidNight(_ t:Double)->Double {
        let T = equationOfTime(jDate + t)
    ...
    private func computeTimes(_ times:[TimeNames: Double])->[TimeNames: Double] {
        ...
        let midnight: Double = computeMidNight(t[.Midnight]!)
    ...
    private func computeDayTimes()->[TimeNames: Any] {
        //default times
        let times = PrayerTimes.DefaultDayTimes;
